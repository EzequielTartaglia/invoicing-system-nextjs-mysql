'use client'
import DeleteAllProductIdButton from "../../../app/sales/[id]/DeleteAllProductIdButton"
...
export default function SaleItemProductsTableWithPaginator({validSaleItems, saleId, sale_is_closed}) {
  ...
  const visibleSaleItems = validSaleItems.slice(startIndex, endIndex);

  const totalPages = Math.ceil(validSaleItems.length / itemsPerPage);
  ...
  const goToLastPage = () => {
    ...
    <thead className="bg-blue-500 w-full text-white font-bold">
      <tr>
        ...
        {sale_is_closed == 0 && <th className="py-1 px-6 border-b">Acciones</th>}
      </tr>
    </thead>
    <tbody>
      {
        (visibleSaleItems.length > 0) &&
        visibleSaleItems.map((sale_item) => (
        <tr key={sale_item.product_id}  className="hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700">
          ...
          {sale_is_closed == 0 && <td className="py-1 px-6 border-b text-center"><DeleteAllProductIdButton saleId={saleId} productId={sale_item.product_id}/></td>}         
        </tr>
      ))
      }
    </tbody>
